...
router.get('/', function(req, res, next) {
  ...
  stringData = sanFran;
  request.get({ uri:'https://coinmap.org/api/v1/venues/?' + stringData }, function (error, response, body) {
    if (!(error && response.statusCode !== 200)) {
      //parse into json object and then get only the sf venues
      venues.sf = JSON.parse(body).venues;
      ...
      request.get({ uri:'https://coinmap.org/api/v1/venues/?' + stringData }, function (error, response, body) {
        if (!(error && response.statusCode !== 200)) {
          ...
          venues.chi = JSON.parse(body).venues;
